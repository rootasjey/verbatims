 ...
 * Handles database operations for backup files
 */
...
export async function createBackupFile(
  ...
  try {
    ...
    const expiresAt = data.retention_days 
      ? new Date(Date.now() + data.retention_days * 24 * 60 * 60 * 1000).toISOString()
      : null
 ...
 */
export async function updateBackupFileStatus(
  db: any,
  backupId: number,
...
/**
 * Get backup file by file key
 */
export async function getBackupFileByKey(
  db: any, 
  fileKey: string
): Promise<BackupFile | null> {
  try {
    const result = await db.prepare(`
      SELECT * FROM backup_files WHERE file_key = ?
    `).bind(fileKey).first()
...
/**
 * Get backup files for export log
 */
export async function getBackupFilesForExport(
  db: any, 
  exportLogId: number
): Promise<BackupFile[]> {
  try {
...
/**
 * List backup files with pagination and filtering
 */
export async function listBackupFiles(
  ...
  try {
    ...
    if (status) {
      conditions.push('bf.storage_status = ?')
      bindings.push(status)
    }
    
    if (dateFrom) {
      conditions.push('bf.created_at >= ?')
...
/**
 * Update backup file access tracking
 */
export async function trackBackupFileAccess(
  db: any, 
  backupId: number
): Promise<void> {
  try {
 ...
 */
export async function deleteBackupFileRecord(
  db: any, 
  backupId: number
): Promise<boolean> {
  try {
...
/**
 * Ensure backup_files table exists (for migration compatibility)
 */
