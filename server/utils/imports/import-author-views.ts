...
export async function importAuthorViewsInline(importId: string, data: any[], options?: ImportOptions): Promise<void> {
  ...
  for (const row of data) {
    try {
      await db.prepare('INSERT OR IGNORE INTO author_views (id, author_id, user_id, viewed_at, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?)')
        .bind(row.id, row.author_id, row.user_id || null, row.viewed_at || null, row.ip_address || null, row.user_agent || null).run()
      success++
    } catch (e: any) {
      failed++
