...
defineRouteMeta({
  openAPI: {
    summary: 'Search filter values',
    description: 'Search-as-you-type endpoint for filter values (tags, authors, references, keywords, etc.).',
    tags: ['Themes'],
    security: [{ apiKey: ['admin:themes'] }],
    parameters: [
      { name: 'q', in: 'query', required: true, schema: { type: 'string' } },
      { name: 'type', in: 'query', required: true, schema: { type: 'string', enum: ['tag_name', 'author_name', 'reference_name', 'author_id', 'reference_id', 'keyword', 'language'] } },
    ],
    responses: {
      '200': { description: 'Matching filter values' },
    },
  },
})
