  ...
  <div class="min-h-screen">
    <div v-if="pending" class="flex items-center justify-center py-16">
      <div class="flex items-center gap-3">
        ...
        <span class="text-gray-600 dark:text-gray-400">{{ $t('tag_detail_loading') }}</span>
      </div>
    </div>
      ...
      <header class="mt-12 p-8 text-center">
        <div class="flex items-center justify-center mb-4">
          <NBadge badge="soft" size="sm" :style="tagBadgeStyle">
            {{ tag.category || $t('tag_detail_general_label') }}
          </NBadge>
        </div>
        ...
        <p class="font-sans text-sm text-gray-500 dark:text-gray-400">
           {{ tag.usage_count || 0 }} {{ (tag.usage_count || 0) === 1 ? $t('tag_detail_quote_singular') : $t('tag_detail_quote_plural') }} tagged
        </p>
      </header>
      ...
      <div class="px-8 pb-16">
        <div class="font-body mb-8">
          <div class="hidden md:flex gap-4 max-w-2xl mx-auto items-center justify-center">
            ...
            <span>•</span>
            <span class="whitespace-nowrap font-600 text-gray-600 dark:text-gray-500">{{ $t('tag_detail_sorted_by') }}</span>
            <NSelect
              v-model="sortByModel"
              :items="sortOptions"
              :placeholder="$ts('tag_detail_sort_placeholder')"
              item-key="label"
              value-key="label"
              @change="loadQuotes"
            />
        ...
        <div v-else class="text-center py-16">
          ...
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ $t('tag_detail_empty') }}</h3>
        ...
        <div v-if="hasMoreQuotes && !quotesLoading" class="flex justify-center">
          <LoadMoreButton
            class="mb-4"
            :idleText="$ts('tag_detail_load_more')"
            :loadingText="$ts('tag_detail_loading_more')"
    ...
    <div v-else class="p-8">
      <div class="text-center py-16">
        ...
        <h2 class="font-title text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ $ts('tag_detail_not_found_title') }}</h2>
        <p class="font-serif text-gray-600 dark:text-gray-400 mb-6">
          {{ $ts('tag_detail_not_found_desc') }}
        </p>
        <NButton to="/tags" size="sm" btn="solid-black" class="px-8 py-3 rounded-3">
          {{ $ts('tag_detail_browse') }}
        </NButton>
      </div>
    </div>
...
const sortOptions = [
  { label: $ts('tag_detail_sort_most_recent'), value: 'created_at' },
  { label: $ts('tag_detail_sort_most_liked'), value: 'likes_count' },
  { label: $ts('tag_detail_sort_most_viewed'), value: 'views_count' },
  { label: $ts('tag_detail_sort_most_shared'), value: 'shares_count' }
]
...
useVerbatimsSeo(() => {
  ...
  if (!tagName) {
    return {
      title: $ts('tag_detail_meta_title_fallback'),
      description: $ts('tag_detail_meta_desc_fallback'),
      type: 'website'
    }
  }
  return {
