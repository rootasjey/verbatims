<template>
  <AppDialog
    v-model="isOpen"
    :title="$t('admin_social.meta_oauth_title') as string"
  ...
  >
    <p class="text-sm text-gray-400 dark:text-gray-400 mb-6">
      {{ $t('admin_social.meta_config_desc') }}
    </p>
    ...
    <div class="space-y-6">
      ...
      >
        <template #trailing>
          <NBadge size="xs" badge="soft-gray" rounded="1" class="py-0.5 text-sm">{{ $t('admin_social.meta_app_id') }}</NBadge>
        </template>
      </NInput>
      ...
      <NInput
        ...
        :placeholder="$t('admin_social.meta_redirect_placeholder') as string"
        class="bg-white dark:bg-gray-900 b-none shadow-none"
        :una="{ inputTrailingWrapper: 'pr-1.5' }"
      >
        <template #trailing>
